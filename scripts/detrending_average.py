...
def generate_plot():
    ...
    median = moving_average(data, median_window)
    ...
    ax1.plot(median, c="red", lw=0.85, label=r"$x_\mathrm{a}(k)$ -- \textit{średnia z okna M}")
    ax1.set_ylabel(r'$x(k)$')
    ax1.legend()
    ax2.axhline(0, color='grey', lw=0.75, ls='--')
    ax2.plot(detrended, c="black", lw=0.75, label=r'$y(k) = x_\mathrm{f}(k) - x_\mathrm{a}(k)$')
