...
def generate_plot():
    setup_latex_plots()
    data = np.load("src/correlation-adaptive.npz")
    ...
    fig, (ax0, ax1) = plt.subplots(2, 1, figsize=(TEXT_WIDTH, 0.75*TEXT_WIDTH), sharex=True, gridspec_kw={'height_ratios': [1/1.618, 1]})
    ...
    ax1_pinheight = np.max(correlation_signal) * 1.1
    ax1_minuspinheight = np.min(correlation_signal) * 1.1
    t1, = ax1.plot(up_threshold, c="green", ls="--", lw=0.75)
    __, = ax1.plot(bottom_threshold, c="green", ls="--", lw=0.75)
    ax1.vlines(fsm_enter_ts, 0, ax1_pinheight, colors='green', linewidth=1.0)
    s1 = ax1.scatter(fsm_enter_ts, [ax1_pinheight] * len(fsm_enter_ts), color='green', marker='o', s=20, zorder=5)
    ax1.vlines(fsm_leave_ts, ax1_minuspinheight, 0, colors='red', linewidth=1.0)
    s2 = ax1.scatter(fsm_leave_ts, [ax1_minuspinheight] * len(fsm_leave_ts), color='red', marker='o', s=20, zorder=5)
    ax1.set_ylabel(r'$r_{wy}(k)$')
    ax1.legend([p, t1, (s1, s2)], [r'$r_{wy}(k)$', r'$\gamma(k)$', "Detekcja"], 
           handler_map={tuple: HandlerTuple(ndivide=None)},
           loc='upper right')
    ax1.set_xlabel(r'$k$')
    ax1.set_xlim((0, 11500))
    p, = ax1.plot(correlation_signal, c="black", lw=0.75)
